...
public sealed class PythonInferenceService : IDisposable
{
    ...
    public PythonInferenceService(ILogger<PythonInferenceService> logger, PythonGilExecutor gil)
    {
        _logger = logger;
        _gil = gil;
    }
        ...
        bool m5IncludesForming = true)
    {
        ...
        var resp = await RunCliAsync(req, timeout ?? TimeSpan.FromSeconds(90), ct).ConfigureAwait(false);
        if (resp is null)
            throw new TimeoutException($"智能体推理超时 >{(timeout ?? TimeSpan.FromSeconds(90)).TotalSeconds:F0}s");
        ...
        _logger.LogInformation("子进程智能体完成 {Count} 条 {Ms}ms", list.Count, sw.ElapsedMilliseconds);
    ...
    public async Task AgentValidateAsync(string configPath, CancellationToken ct = default)
    {
        ...
        var resp = await RunCliAsync(req, TimeSpan.FromSeconds(120), ct).ConfigureAwait(false);
        if (resp is null)
            throw new TimeoutException("智能体环境验证超时");
        if (!resp.Value.GetProperty("ok").GetBoolean())
            throw new InvalidOperationException(ReadCliError(resp) ?? "智能体环境验证失败");
    }
    ...
    public bool ValidateModels(IEnumerable<string> symbols)
    {
        ...
        try
        {
            var resp = RunCliAsync(new { cmd = "validate", symbols = symbols.ToArray() },
    ...
    private async Task<JsonElement?> RunCliAsync(object request, TimeSpan timeout, CancellationToken ct)
    {
        ...
        var script = Path.Combine(AppPaths.PythonEngineDir, "inference_cli.py");
    ...
    private static void ApplySubprocessEnv(ProcessStartInfo psi)
    {
        ...
        // ⚠ 字典初始不含 PYTHONPATH，必须用 TryGetValue 避免 KeyNotFoundException
        var exists = psi.Environment.TryGetValue("PYTHONPATH", out var existing);
        var paths = new List<string> { installDir, engineDir };
        ...
        var pathParts = new List<string> { installDir };
