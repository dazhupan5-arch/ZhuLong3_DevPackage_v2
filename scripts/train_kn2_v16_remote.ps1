<#
.SYNOPSIS
  KN2 V16 跨机 GPU 训练脚本（在 RTX 3050 等 GPU 机器上运行）
...
.PARAMETER BatchSize
  序列 batch 大小；RTX 3050 8GB 建议 32–48，OOM 改 24
...
"@ -ForegroundColor Yellow
    ...
    py -3 -c "import numpy as np; d=np.load(r'$npz'); print('rows', d['market_feat'].shape[0], 'dim', d['market_feat'].shape[1])"
    Write-Host "npz size: ${sizeMb} MB" -ForegroundColor Green
}

Write-Step "KN2 V16 训练 (mode=fast, batch=$BatchSize)"
$logDir = Join-Path $Root "logs"
New-Item -ItemType Directory -Force -Path $logDir | Out-Null
$log = Join-Path $logDir ("kn2_v16_remote_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".log")
