#!/usr/bin/env python3
"""KN 2.0 v3 — 3-action model (hold/long/short), 192d GRU x2, aggressive class weights."""
...
# Simple 3-action model is much easier to train
HIDDEN_DIM, NUM_LAYERS, EMBED_DIM = 256, 2, 64
NUM_ACTIONS = 3  # hold/long/short only
EPOCHS, BATCH_SEQ, PATIENCE, SEQ_LEN = 100, 20, 30, 64
LR_BASE = 0.0005
CLASS_WEIGHTS = [1.0, 3.0, 0.3]  # high weight=model cares more → encourage long, discourage short
...
def gen_labels(df, atr_v):
    ...
    d=np.bincount(a,minlength=3); log(f"  Done: {d} trade={st.mean()*100:.1f}%")
...
def eval_model(path, vm, vl):
    ...
    ll=vl["action"][:len(p)]
    # Clip to 0-2 range since model may output >2
    p=np.clip(p,0,2)
    ...
    log(f"  Dist: h={pd[0]*100:.0f}% l={pd[1]*100:.0f}% s={pd[2]*100:.0f}%")
...
def main():
    log("="*60)
    log(f"KN2 v3 | {HIDDEN_DIM}d GRU x{NUM_LAYERS} | {NUM_ACTIONS} actions | Weights={CLASS_WEIGHTS}")
    log(f"  Data: {TRAIN_YEARS} ({BARS_LIMIT:,}) | Val: {VAL_YEAR} | Pass: acc>50% prec>30% dist>10%")
    log("="*60)
