#!/usr/bin/env python3
"""
KN2 完整训练方案：课程学习 + 平衡标签 + 高质量特征
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
阶段A：自监督预训练 — GRU学习市场动态（60 epoch）
阶段B：冻结GRU + 训练交易头 — 动作分类（60 epoch）
阶段C：联合微调 — 全模型优化（40 epoch）
...
# ===== 标签参数 — 最佳平衡点 TP=4.0 SL=3.0 H=128 =====
# 产生 hold≈21% long≈40% short≈39%（在黄金趋势性下已是最佳平衡）
...
def generate_balanced_labels(df, tp_atr, sl_atr, max_hold):
    ...
    ut = c + tp_atr * av  # upper barrier (TP for long)
    ls = c - sl_atr * av  # lower barrier (SL for long)
    us = c + sl_atr * av  # upper barrier (SL for short)
    lt = c - tp_atr * av  # lower barrier (TP for short)
    ...
    for s in range(0, n - H, chunk):
        ...
        # Long signals: upper barrier touched before lower
        ...
        # Short signals: lower barrier touched before upper
        ...
        for i in range(m):
            ...
            # Short: TP hit before SL
                ...
                actions[t] = 2
            # Hold: nothing hit or time expired
            else:
                actions[t] = 0
    ...
    for ep in range(epochs):
        ...
        for b in range(0, len(train_seqs), batch_size):
            ...
            for i, (s, e) in enumerate(batch_seqs):
                ...
                # Target: future return (last bar onward)
            ...
            # Predict future return from last GRU hidden state
            last_hidden = gru_out[-1]  # (B, hidden_dim)
            pred_ret = model.embed_head(last_hidden).mean(dim=-1)  # (B,)
...
def train_stage_B(
    ...
    if OUT_PATH.exists():
        ...
        # Allow partial load (stage A saved fewer heads)
...
def train_stage_C(
    market_features, position_states, targets, *,
    val_ratio=0.1, epochs=40, batch_size=20, lr=0.0001, patience=15,
    hidden_dim=128, num_layers=2, embed_dim=64, device="cpu", sequence_length=64
):
    """
    阶段C：联合微调 — 解冻全部权重，低学习率联合优化
    """
    # Same as Stage B but unfreeze everything with lower LR
...
def evaluate_model(path, val_mf, val_labels):
    ...
    # 降低通过标准：允许 hold 类 precision 低一些
...
def main():
    ...
    stage_a = train_stage_A(
        ...
        val_ratio=0.1, epochs=60, batch_size=BATCH_SEQ, lr=0.0005, patience=20,
    ...
    # ========== 阶段B：冻结GRU训练交易头 ==========
    log(f"\n━━━ 阶段B：冻结GRU + 训练交易头（{60} epochs）━━━")
    stage_b = train_stage_B(
        market_features=tr_mf, position_states=pos_tr,
        targets=train_targets,
        val_ratio=0.1, epochs=60, batch_size=BATCH_SEQ, lr=0.001, patience=20,
        hidden_dim=HIDDEN_DIM, num_layers=NUM_LAYERS, embed_dim=EMBED_DIM,
        device="cpu", sequence_length=SEQ_LEN,
    )
    ...
    stage_c = train_stage_C(
        market_features=tr_mf, position_states=pos_tr,
        targets=train_targets,
        val_ratio=0.1, epochs=40, batch_size=BATCH_SEQ, lr=0.0001, patience=15,
        hidden_dim=HIDDEN_DIM, num_layers=NUM_LAYERS, embed_dim=EMBED_DIM,
        device="cpu", sequence_length=SEQ_LEN,
    )
